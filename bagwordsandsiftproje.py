...
# Her sınıf için belirli sayıda görüntü kullanmak için bir sayaç oluştur
image_counter = {}
...
for class_folder in os.listdir(data_folder):
    ...
    if os.path.isdir(class_path):
        ...
        for image_file in os.listdir(class_path):
            if image_counter[class_folder] < 200:  # Her sınıf için 100 görüntüyü kullan
...
def extract_sift_features(image):
    ...
    if descriptors is not None:
        descriptors = descriptors[:128, :].astype(np.float32)  # Veri tipini float32'ye dönüştür
...
# build_bow fonksiyonunu güncelle
...
# Test verileri üzerinde sınıflandırma yap ve doğruluk değerini hesapla
